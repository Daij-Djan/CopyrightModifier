 ...
 * Refactoring & mangling: @inquisitivesoft (Harry Jordan), @daijdjan (Dominik Pich)
 */
...
- (void)test_commonPrefixTest
{
	...
	XCTAssertTrue(prefixIndex == 0, @"Common suffix null case failed.");
	
	// Non-null case.
	prefixIndex = diff_commonPrefix((__bridge CFStringRef)@"1234abcdef", (__bridge CFStringRef)@"1234xyz");
	XCTAssertTrue(prefixIndex == 4, @"Common suffix non-null case failed.");
	
	// Whole case.
	prefixIndex = diff_commonPrefix((__bridge CFStringRef)@"1234", (__bridge CFStringRef)@"1234xyz");
	XCTAssertTrue(prefixIndex == 4, @"Common suffix whole case failed.");
}
...
- (void)test_diff_commonSuffixTest
{
	...
	XCTAssertTrue(prefixIndex == 0, @"Detect any common suffix. Null case.");
	
	// Non-null case.
	prefixIndex = diff_commonSuffix((__bridge CFStringRef)@"abcdef1234", (__bridge CFStringRef)@"xyz1234");
	XCTAssertTrue(prefixIndex == 4, @"Detect any common suffix. Non-null case.");
	
	// Whole case.
	prefixIndex = diff_commonSuffix((__bridge CFStringRef)@"1234", (__bridge CFStringRef)@"xyz1234");
	XCTAssertTrue(prefixIndex == 4, @"Detect any common suffix. Whole case.");
}
...
- (void)test_diff_halfmatchTest
{
	...
	// No match.
	XCTAssertNil((__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"1234567890", (__bridge CFStringRef)@"abcdef"), @"No match #1.");
	
	XCTAssertNil((__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"12345", (__bridge CFStringRef)@"23"), @"No match #2.");
	
	// Single Match.
	NSArray *expectedResult = @[@"12", @"90", @"a", @"z", @"345678"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"1234567890", (__bridge CFStringRef)@"a345678z"), @"Single Match #1.");
	
	expectedResult = @[@"a", @"z", @"12", @"90", @"345678"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"a345678z", (__bridge CFStringRef)@"1234567890"), @"Single Match #2.");
	
	expectedResult = @[@"abc", @"z", @"1234", @"0", @"56789"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"abc56789z", (__bridge CFStringRef)@"1234567890"), @"Single Match #3.");
	
	expectedResult = @[@"a", @"xyz", @"1", @"7890", @"23456"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"a23456xyz", (__bridge CFStringRef)@"1234567890"), @"Single Match #4.");
	
	// Multiple Matches.
	expectedResult = @[@"12123", @"123121", @"a", @"z", @"1234123451234"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"121231234123451234123121", (__bridge CFStringRef)@"a1234123451234z"), @"Multiple Matches #1.");
	
	expectedResult = @[@"", @"-=-=-=-=-=", @"x", @"", @"x-=-=-=-=-=-=-="];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"x-=-=-=-=-=-=-=-=-=-=-=-=", (__bridge CFStringRef)@"xx-=-=-=-=-=-=-="), @"Multiple Matches #2.");
	
	expectedResult = @[@"-=-=-=-=-=", @"", @"", @"y", @"-=-=-=-=-=-=-=y"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"-=-=-=-=-=-=-=-=-=-=-=-=y", (__bridge CFStringRef)@"-=-=-=-=-=-=-=yy"), @"Multiple Matches #3.");
	
	// Non-optimal halfMatch.
	// Optimal diff would be -q+x=H-i+e=lloHe+Hu=llo-Hew+y not -qHillo+x=HelloHe-w+Hulloy
	expectedResult = @[@"qHillo", @"w", @"x", @"Hulloy", @"HelloHe"];
	XCTAssertEqualObjects(expectedResult, (__bridge_transfer NSArray *)diff_halfMatchCreate((__bridge CFStringRef)@"qHilloHelloHew", (__bridge CFStringRef)@"xHelloHeHulloy"), @"Non-optimal halfmatch.");
...
- (void)test_diff_charsToLinesTest {
	...
	XCTAssertEqualObjects(@[[DMDiff diffWithOperation:DIFF_DELETE andText:lines]], diffs, @"More than 256 #3.");
}
...
- (void)test_diff_prettyHtmlTest
{
	...
	XCTAssertEqualObjects(expectedResult, actualResult, @"Pretty print.");
}
...
- (void)test_diff_textTest {
	...
	XCTAssertEqualObjects(@"jumps over the lazy", diff_text1(diffs), @"Compute the source and destination texts #1");
	XCTAssertEqualObjects(@"jumped over a lazy", diff_text2(diffs), @"Compute the source and destination texts #2");
}
...
- (void)test_diff_deltaTest
{
	...
	XCTAssertEqualObjects(diffs, diff_diffsFromOriginalTextAndDelta(text1, delta, NULL), @"Convert delta string into a diff.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsFromOriginalTextAndDelta(text1, delta, NULL), @"diff_fromDelta: Unicode 2.");
...
- (void)test_diff_translateLocationFromText1ToText2
{
	...
	XCTAssertEqual((NSUInteger)5, diff_translateLocationFromText1ToText2(diffs, 2), @"diff_translateLocationFromText1ToText2: Translation on equality. Translate a location in text1 to text2.");
	...
	XCTAssertEqual((NSUInteger)1, diff_translateLocationFromText1ToText2(diffs, 3), @"diff_translateLocationFromText1ToText2: Translation on deletion.");
}
...
- (void)test_diff_levenshteinTest
{
	...
	XCTAssertEqual((NSUInteger)4, diff_levenshtein(diffs), @"diff_levenshtein: Levenshtein with trailing equality.");
	...
	XCTAssertEqual((NSUInteger)4, diff_levenshtein(diffs), @"diff_levenshtein: Levenshtein with leading equality.");
	...
	XCTAssertEqual((NSUInteger)7, diff_levenshtein(diffs), @"diff_levenshtein: Levenshtein with middle equality.");
}
...
- (void)diff_bisectTest;
{
	...
	XCTAssertEqualObjects(diffs, diff_bisectOfStrings(a, b, properties), @"Bisect test.");
	...
	XCTAssertEqualObjects(diffs, diff_bisectOfStrings(a, b, properties), @"Bisect timeout.");
}
...
- (void)test_diff_mainTest
{
	...
	NSArray *diffs = @[];
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"", @""), @"diff_main: Null case.");
	
	
	diffs = @[[DMDiff diffWithOperation:DIFF_EQUAL andText:@"abc"]];
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"abc", @"abc"), @"diff_main: Equality.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"abc", @"ab123c"), @"diff_main: Simple insertion.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"a123bc", @"abc"), @"diff_main: Simple deletion.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"abc", @"a123b456c"), @"diff_main: Two insertions.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTexts(@"a123b456c", @"abc"), @"diff_main: Two deletions.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"a", @"b", TRUE, 0.0), @"diff_main: Simple case #1.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"Apples are a fruit.", @"Bananas are also fruit.", TRUE, 0.0), @"diff_main: Simple case #2.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"ax\t", [NSString stringWithFormat:@"\U00000680x%C", (unichar)0], TRUE, 0.0), @"diff_main: Simple case #3.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"1ayb2", @"abxab", TRUE, 0.0), @"diff_main: Overlap #1.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"abcy", @"xaxcxabc", TRUE, 0.0), @"diff_main: Overlap #2.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"ABCDa=bcd=efghijklmnopqrsEFGHIJKLMNOefg", @"a-bcd-efghijklmnopqrs", TRUE, 0.0), @"diff_main: Overlap #3.");
	...
	XCTAssertEqualObjects(diffs, diff_diffsBetweenTextsWithOptions(@"a [[Pennsylvania]] and [[New", @" and [[Pennsylvania]]", TRUE, 0.0), @"diff_main: Large equality.");
...
//	dmp.Diff_Timeout = 0.1f;  // 100ms
	...
	XCTAssertTrue((timeLimit <= (endTime - startTime)), @"Test that we took at least the timeout period.");
	// Test that we didn't take forever (be forgiving).
	// Theoretically this test could fail very occasionally if the
	// OS task swaps or locks up for a second at the wrong moment.
	// This will fail when running this as PPC code thru Rosetta on Intel.
	XCTAssertTrue(((timeLimit * 2) > (endTime - startTime)), @"Test that we didn't take forever (be forgiving).");
	...
	XCTAssertEqualObjects(diff_diffsBetweenTextsWithOptions(a, b, FALSE, 0.0), diff_diffsBetweenTextsWithOptions(a, b, TRUE, 0.0), @"diff_main: Simple line-mode.");
	
	a = @"1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890";
	b = @"abcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghij";
	XCTAssertEqualObjects(diff_diffsBetweenTextsWithOptions(a, b, FALSE, 0.0), diff_diffsBetweenTextsWithOptions(a, b, TRUE, 0.0), @"diff_main: Single line-mode.");
	...
	// Test null inputs
	XCTAssertNil(diff_diffsBetweenTexts(NULL, NULL), @"Test null inputs. #1");
	XCTAssertNil(diff_diffsBetweenTexts(@"a", NULL), @"Test null inputs. #2");
	XCTAssertNil(diff_diffsBetweenTexts(NULL, @"b"), @"Test null inputs. #3");
}
...
- (void)test_match_alphabetTest {
	...
	XCTAssertEqualObjects(bitmask, match_alphabetFromPattern(@"abc"), @"match_alphabet: Unique.");
	
	bitmask = [@{ @"a" : @(37), @"b" : @(18), @"c" : @(8) } mutableCopy];
	XCTAssertEqualObjects(bitmask, match_alphabetFromPattern(@"abcaba"), @"match_alphabet: Duplicates.");
}
...
- (void)test_match_bitapTest
{
	...
	XCTAssertEqual((NSUInteger)5, match_bitapOfTextAndPattern(@"abcdefghijk", @"fgh", 5, properties), @"match_bitap: Exact match #1.");
	XCTAssertEqual((NSUInteger)5, match_bitapOfTextAndPattern(@"abcdefghijk", @"fgh", 5, properties), @"match_bitap: Exact match #2.");
	XCTAssertEqual((NSUInteger)4, match_bitapOfTextAndPattern(@"abcdefghijk", @"efxhi", 0, properties), @"match_bitap: Fuzzy match #1.");
	XCTAssertEqual((NSUInteger)2, match_bitapOfTextAndPattern(@"abcdefghijk", @"cdefxyhijk", 5, properties), @"match_bitap: Fuzzy match #2.");
	XCTAssertEqual((NSUInteger)NSNotFound, match_bitapOfTextAndPattern(@"abcdefghijk", @"bxy", 1, properties), @"match_bitap: Fuzzy match #3.");
	XCTAssertEqual((NSUInteger)2, match_bitapOfTextAndPattern(@"123456789xx0", @"3456789x0", 2, properties), @"match_bitap: Overflow.");
	XCTAssertEqual((NSUInteger)0, match_bitapOfTextAndPattern(@"abcdef", @"xxabc", 4, properties), @"match_bitap: Before start match.");
	XCTAssertEqual((NSUInteger)3, match_bitapOfTextAndPattern(@"abcdef", @"defyy", 4, properties), @"match_bitap: Beyond end match.");
	XCTAssertEqual((NSUInteger)0, match_bitapOfTextAndPattern(@"abcdef", @"xabcdefy", 0, properties), @"match_bitap: Oversized pattern.");
	
	properties.matchThreshold = 0.4f;
	XCTAssertEqual((NSUInteger)4, match_bitapOfTextAndPattern(@"abcdefghijk", @"efxyhi", 1, properties), @"match_bitap: Threshold #1.");
	
	properties.matchThreshold = 0.3f;
	XCTAssertEqual((NSUInteger)NSNotFound, match_bitapOfTextAndPattern(@"abcdefghijk", @"efxyhi", 1, properties), @"match_bitap: Threshold #2.");

	properties.matchThreshold = 0.0f;
	XCTAssertEqual((NSUInteger)1, match_bitapOfTextAndPattern(@"abcdefghijk", @"bcdef", 1, properties), @"match_bitap: Threshold #3.");
	
	properties.matchThreshold = 0.5f;
	XCTAssertEqual((NSUInteger)0, match_bitapOfTextAndPattern(@"abcdexyzabcde", @"abccde", 3, properties), @"match_bitap: Multiple select #1.");
	XCTAssertEqual((NSUInteger)8, match_bitapOfTextAndPattern(@"abcdexyzabcde", @"abccde", 5, properties), @"match_bitap: Multiple select #2.");
	
	properties.matchDistance = 10;  // Strict location.
	XCTAssertEqual((NSUInteger)NSNotFound, match_bitapOfTextAndPattern(@"abcdefghijklmnopqrstuvwxyz", @"abcdefg", 24, properties), @"match_bitap: Distance test #1.");
	XCTAssertEqual((NSUInteger)0, match_bitapOfTextAndPattern(@"abcdefghijklmnopqrstuvwxyz", @"abcdxxefg", 1, properties), @"match_bitap: Distance test #2.");
	
	properties.matchDistance = 1000;  // Loose location.
	XCTAssertEqual((NSUInteger)0, match_bitapOfTextAndPattern(@"abcdefghijklmnopqrstuvwxyz", @"abcdefg", 24, properties), @"match_bitap: Distance test #3.");
}
...
{	
	// Full match.
	XCTAssertEqual((NSUInteger)0, match_locationOfMatchInText(@"abcdef", @"abcdef", 1000), @"match_main: Equality.");
	XCTAssertEqual((NSUInteger)NSNotFound, match_locationOfMatchInText(@"", @"abcdef", 1), @"match_main: Null text.");
	XCTAssertEqual((NSUInteger)3, match_locationOfMatchInText(@"abcdef", @"", 3), @"match_main: Null pattern.");
	XCTAssertEqual((NSUInteger)3, match_locationOfMatchInText(@"abcdef", @"de", 3), @"match_main: Exact match.");
	XCTAssertEqual((NSUInteger)3, match_locationOfMatchInText(@"abcdef", @"defy", 4), @"match_main: Beyond end match.");
	XCTAssertEqual((NSUInteger)0, match_locationOfMatchInText(@"abcdef", @"abcdefy", 0), @"match_main: Oversized pattern.");
	XCTAssertEqual((NSUInteger)4, match_locationOfMatchInTextWithOptions(@"I am the very model of a modern major general.", @" that berry ", 5, 0.7f, 1000), @"match_main: Complex match.");
	
	XCTAssertEqual((NSUInteger)NSNotFound, match_locationOfMatchInText(NULL, @"Pattern", 0), @"match_main: Null text.");
	XCTAssertEqual((NSUInteger)NSNotFound, match_locationOfMatchInText(@"Text", NULL, 0), @"match_main: Null pattern.");
	XCTAssertEqual((NSUInteger)NSNotFound, match_locationOfMatchInText(NULL, NULL, 0), @"match_main: Both null.");
}
...
- (void)test_patch_patchObjTest {
	...
	XCTAssertEqualObjects(expectedDescription, [patch patchText], @"Patch: description.");
}
...
- (void)test_patch_fromTextTest
{
	...
	XCTAssertTrue(patch_parsePatchesFromText(@"", NULL).count == 0, @"patch_fromText: #0.");
	
	NSString *patchesAsText = @"@@ -21,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n %0Alaz\n";
	XCTAssertEqualObjects(patchesAsText, [[patch_parsePatchesFromText(patchesAsText, NULL) objectAtIndex:0] patchText], @"patch_fromText: #1.");
	
	patchesAsText = @"@@ -1 +1 @@\n-a\n+b\n";
	XCTAssertEqualObjects(patchesAsText, [[patch_parsePatchesFromText(patchesAsText, NULL) objectAtIndex:0] patchText], @"patch_fromText: #2.");
	
	patchesAsText = @"@@ -1,3 +0,0 @@\n-abc\n";
	XCTAssertEqualObjects(patchesAsText, [[patch_parsePatchesFromText(patchesAsText, NULL) objectAtIndex:0] patchText], @"patch_fromText: #3.");
	
	patchesAsText = @"@@ -0,0 +1,3 @@\n+abc\n";
	XCTAssertEqualObjects(patchesAsText, [[patch_parsePatchesFromText(patchesAsText, NULL) objectAtIndex:0] patchText], @"patch_fromText: #4.");
...
- (void)test_patch_toTextTest
{
	...
	XCTAssertEqualObjects(patchesAsText, patch_patchesToText(patches), @"toText Test #1");
	
	patchesAsText = @"@@ -1,9 +1,9 @@\n-f\n+F\n oo+fooba\n@@ -7,9 +7,9 @@\n obar\n-,\n+.\n  tes\n";
	patches = patch_parsePatchesFromText(patchesAsText, NULL);
	XCTAssertEqualObjects(patchesAsText, patch_patchesToText(patches), @"toText Test #2");
}
...
- (void)test_patch_makeTest
{
	...
	XCTAssertEqualObjects(@"", patch_patchesToText(patches), @"patch_patchesFromTexts: Empty case.");

	patches = patch_patchesFromTexts(nil, nil);
	XCTAssertEqualObjects(@"", patch_patchesToText(patches), @"patch_patchesFromTexts: Nil case.");
	...
	XCTAssertEqualObjects(patchAsText, patch_patchesToText(patches), @"patch_patchesFromTexts: Text2+Text1 inputs.");
	
	patchAsText = @"@@ -1,11 +1,12 @@\n Th\n-e\n+at\n  quick b\n@@ -22,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n  laz\n";
	patches = patch_patchesFromTexts(text1, text2);
	XCTAssertEqualObjects(patchAsText, patch_patchesToText(patches), @"patch_patchesFromTexts: Text1+Text2 inputs.");
	
	PatchProperties properties = patch_defaultPatchProperties();
	NSArray *diffs = diff_diffsBetweenTexts(text1, text2);
	patches = patch_patchesFromDiffs(diffs, properties);
	XCTAssertEqualObjects(patchAsText, patch_patchesToText(patches), @"patch_patchesFromDiffs: Diff input.");
	
	patches = patch_patchesFromTextAndDiffs(text1, diffs, properties);
	XCTAssertEqualObjects(patchAsText, patch_patchesToText(patches), @"patch_patchesFromTextAndDiffs: Text1+Diff inputs.");
	...
	XCTAssertEqualObjects(diffs, [(DMPatch *)[patch_parsePatchesFromText(patchAsText, NULL) objectAtIndex:0] diffs], @"patch_fromText: Character decoding.");
	...
	patches = patch_patchesFromTexts(text1, text2);
	XCTAssertEqualObjects(patchAsText, patch_patchesToText(patches), @"patch_make: Long string with repeats.");
}
...
- (void)test_patch_splitMaxTest
{
	...
	patch_splitMax(&mutablePatches, properties);
	XCTAssertEqualObjects(previousText, patch_patchesToText(mutablePatches), @"Assumes that Match_MaxBits is 32 #2");
...
- (void)test_patch_applyTest
{
	...
	XCTAssertEqualObjects(@"Hello world.\t0", resultString, @"patch_apply: Null case.");
	...
	XCTAssertTrue([indexesOfAppliedPatches containsIndexesInRange:NSMakeRange(0, 2)], @"patch_apply: Exact match. Correct indices");
	XCTAssertEqualObjects(@"That quick brown fox jumped over a lazy dog.", resultString, @"patch_apply: Exact match. Text");
	...
	XCTAssertTrue([indexesOfAppliedPatches containsIndexesInRange:NSMakeRange(0, 2)], @"patch_apply: Partial match");
	XCTAssertEqualObjects(@"That quick red rabbit jumped over a tired tiger.", resultString, @"patch_apply: Partial match.");
	...
	XCTAssertFalse([indexesOfAppliedPatches containsIndex:0] && [indexesOfAppliedPatches containsIndex:1], @"patch_apply: Failed match");
	XCTAssertEqualObjects(@"I am the very model of a modern major general.", resultString, @"patch_apply: Failed match.");
	...
	XCTAssertTrue([indexesOfAppliedPatches containsIndexesInRange:NSMakeRange(0, 2)], @"patch_apply: Big delete, small change");
	XCTAssertEqualObjects(@"xabcy", resultString, @"patch_apply: Big delete, small change.");
	...
	XCTAssertTrue([indexesOfAppliedPatches count] == 1 && [indexesOfAppliedPatches containsIndex:1], @"patch_apply: Big delete, big change 1.");
	XCTAssertEqualObjects(@"xabc12345678901234567890---------------++++++++++---------------12345678901234567890y", resultString, @"patch_apply: Big delete, big change 1.");
	
	properties.patchDeleteThreshold = 0.6f;
	patches = patch_patchesFromTextsWithProperties(@"x1234567890123456789012345678901234567890123456789012345678901234567890y", @"xabcy", properties);
	resultString = patch_applyPatchesToTextWithProperties(patches, @"x12345678901234567890---------------++++++++++---------------12345678901234567890y", &indexesOfAppliedPatches, properties);
	XCTAssertTrue([indexesOfAppliedPatches containsIndexesInRange:NSMakeRange(0, 2)], @"patch_apply: Big delete, big change 2.");
	XCTAssertEqualObjects(@"xabcy", resultString, @"patch_apply: Big delete, big change 2.");
	...
	XCTAssertTrue([indexesOfAppliedPatches count] == 1 && [indexesOfAppliedPatches containsIndex:1], @"patch_apply: Compensate for failed patch..");
	XCTAssertEqualObjects(@"ABCDEFGHIJKLMNOPQRSTUVWXYZ--------------------1234567YYYYYYYYYY890", resultString, @"patch_apply: Compensate for failed patch.");
	...
	XCTAssertEqualObjects(patchesAsText, patch_patchesToText(patches), @"patch_apply: No side effects.");
	...
	XCTAssertEqualObjects(patchesAsText, patch_patchesToText(patches), @"patch_apply: No side effects with major delete.");
	
	patches = patch_patchesFromTexts(@"", @"test");
	resultString = patch_applyPatchesToText(patches, @"", &indexesOfAppliedPatches);
	XCTAssertTrue([indexesOfAppliedPatches count] == 1 && [indexesOfAppliedPatches containsIndex:0], @"patch_apply: Edge exact match.");
	XCTAssertEqualObjects(@"test", resultString, @"patch_apply: Edge exact match.");
	
	patches = patch_patchesFromTexts(@"XY", @"XtestY");
	resultString = patch_applyPatchesToText(patches, @"XY", &indexesOfAppliedPatches);
	XCTAssertTrue([indexesOfAppliedPatches count] == 1 && [indexesOfAppliedPatches containsIndex:0], @"patch_apply: Near edge exact match.");
	XCTAssertEqualObjects(@"XtestY", resultString, @"patch_apply: Near edge exact match.");
	
	patches = patch_patchesFromTexts(@"y", @"y123");
	resultString = patch_applyPatchesToText(patches, @"x", &indexesOfAppliedPatches);
	XCTAssertTrue([indexesOfAppliedPatches count] == 1 && [indexesOfAppliedPatches containsIndex:0], @"patch_apply: Edge partial match.");
	XCTAssertEqualObjects(@"x123", resultString, @"patch_apply: Edge partial match.");
}
...
- (void)test_speedtests
{
	...
	XCTAssertNotNil(diffs, @"Couldn't genereate diffs for files: %@ - %@", testUrl1, testUrl2);
